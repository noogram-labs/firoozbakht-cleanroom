...
available is $g_n \ll p_n^{0.525}$ \cite{baker2001difference}; under the Riemann Hypothesis
...
the number of primes in an interval of length $y \asymp x/\log x$ that is within a factor
...
deliver is
\[
\frac{\tfrac{22}{25}\sqrt{p_n}L_n}{L_n^2 - L_n - 1} \;\asymp\; \frac{22}{25}\cdot
\frac{\sqrt{p_n}}{L_n} \;\longrightarrow\; \infty ,
\]
...
change; evaluating, the bracket at $x=5$ is $1.6213$ while $\log(5\log 5) = 2.0873$, so
...
unconditional gap bound is $g_n \ll p_n^{0.525}$ \cite{baker2001difference}; under RH it
...
and about $12.8$ decades short of it from the $3\cdot10^6$ scale at which several of the
lemma verifications were performed. \textbf{These sweeps verify the lemmas, not the range.}
